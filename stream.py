...
st.sidebar.title('Video classification')
st.sidebar.title('###')
...
video_file = st.file_uploader("Upload Crowd video")
tfile = tempfile.NamedTemporaryFile(delete=False)

if video_file != None:
    start_time = time.time()
    tfile.write(video_file.read()) #tfile.name

print("[INFO] Starting the video..")
vs = cv2.VideoCapture(tfile.name)
if config.Thread:
    cap = thread.ThreadingClass(tfile.name)
